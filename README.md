# Track-Shop
A full-stack fictional e-commerce music site built as a learning project.
...
-   Ready for production deployment with a [render.yaml](render.yaml) configuration file.
...
## Production
#### TODO: Add remote postgreSQL steps
...
- **Password Security**: Passwords are hashed using `bcrypt` for one-way encryption before being stored in the database, see [`app/services/user.py`](app/services/user.py)
- **Validation**: Strict validation is enforced server-side with Pydantic schemas as defined in [`app/schemas.py`](app/schemas.py). Additional more basic local validation is provided by Bootstrap.
...
1. **Login Page**: The user navigates to `/login` which routes to [`app/routers/users/login.py`](app/routers/users/login.py) as a `GET` request and serves [`app/templates/login.html`](app/templates/login.html)
2. **Login Submission**: The user submits the form which routes to [`app/routers/users/login.py`](app/routers/users/login.py) as a `POST` request. Authentication is checked with [`app/services/user.py`](app/services/user.py). If valid their `Session User ID` is added, creating a local encrypted cookie.
3. **Logged-in state**: Every page loaded can now read this `User ID`, allowing functionality like [`app/templates/base.html`](app/templates/base.html) to toggle between a login and logout button with `{% if request.session.get('user_id') %}`


## Checks
Several checks are included in the `scripts` folder to ensure each step of the app running process is correct.
- [`scripts/check_env.py`](scripts/check_env.py) verifies the local `.env` exists in the root folder
- [`scripts/check_docker.py`](scripts/check_docker.py) verifies the local docker process is running and responds to commands
- [`scripts/check_db.py`](scripts/check_db.py) verifies the local postgreSQL database is running and responds to queries. This helps ensure an adequate delay between `Make` commands 
